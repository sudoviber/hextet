...
- **逐出**：对端换址后，daemon 判定当前连接的旧地址已失效（不再被任何可靠会合源在报）
  时，会把该地址从 `last_good` 与 `seen` 里一并移除——否则死地址会被喂回候选列表、
  让打洞状态机在「死地址 / 活地址」之间来回轮换收敛不了（见 `docs/protocol/punching.md`
  「会合层送来新候选时」）。**逐出同样落盘**（与 `record_good` 同语义）——否则 daemon
  重启会从 endpoints.json 把死 `last_good`/`seen` 读回来，启动时又拿死地址当首候选。
  文件格式不变，仍是 version 1。
