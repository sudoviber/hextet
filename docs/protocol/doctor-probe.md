...
特别地，**当对端自己也在状态防火墙后面时，结论必然是 `blocked`**：客户端发出的
Request 对对端而言是 `ct state new` 的未经请求入站包，会被对端的防火墙丢弃，响应器
根本收不到。因此本协议要求充当探测助手的对端在探针端口上可达。这与 WireGuard 打洞
不同——打洞是两侧同时双向发包、各自建立 state，而探针是单向发起的。放宽这一限制需要
让 Request 从 daemon 自己的探针端口发出，进而需要 daemon IPC（M5，见
