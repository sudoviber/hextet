...
- `Connected` 的 peer：**只换列表，不产生任何动作**——但有一个例外：若新列表**不再
  包含当前连着的地址**、且该地址既不是配置里手填的、也不再是任何一路会合源在报的
  （即对端确实搬走了，而不是"多了一个新地址"），则主动离开旧地址去试新地址
  （`retry_from`）。否则一条正常工作的连接绝不因为"听到了新地址"被打断；新列表会在
  将来握手失效时才起作用。这个例外正是"双侧同时换前缀、只剩 DHT 会合"能秒级恢复的
  关键——否则要等 180s 握手过期才退回 Probing。
