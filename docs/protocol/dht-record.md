...
- **会合层不做身份认证**：会合密钥对是全网共享网络密钥派生的，任何网内成员都能
  ...
  endpoint」，真正的身份认证在 WireGuard 握手（cryptokey routing）完成。伪造不能冒充
  节点，但**不只是一条候选位**：一条记录可塞 35+ 个 endpoint，足以占满对端的全部
  8 个候选槽位（`MAX_CANDIDATES`），并每 30s 查询刷新一次——即**对网内恶意成员而言，
  这是对该 peer 的完整连接 DoS**（非成员被 AEAD+签名挡在门外，做不到）。
