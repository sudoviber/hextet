...
| **gotatun 用户态后端（macOS utun + Linux TUN）** | 🔶 部分完成 | `ADR-0007` 已写；`hextet-platform::tun` TUN 抽象（utun/TUN，`tun` crate 安全封装）+ `hextet-wg-userspace`（boringtun `=0.7.1` 实现 `WgBackend` 全五方法）+ 进程内握手与 IPv6 数据往返测试已完成；`set_peer_endpoint` 已补上「remove + 完整 re-add」重建路径（2026-08-13，见 ADR-0007「代价与再评估」），打洞循环在 boringtun 后端功能可用；`daemon.rs` 的 macOS 完整接线**已实现（2026-08-13，编译验证）**——engine 后端泛化 + `backend::platform_default()` 工厂 + `http::router` 共享后端，打洞循环在 macOS 上走 userspace 后端。剩余阻塞：真实 utun/root 运行时验证（须真机/CI） |
| **launchd 服务（macOS）** | 🔶 打包就绪 / 运行时阻塞 | `packaging/launchd/com.hextet.daemon.plist` + `docs/guides/install.md` macOS 章节（`plutil -lint` 通过）；plist 为「死打包」——`daemon.rs` 的 macOS 接线已实现（2026-08-13，编译验证），真实 utun 需 root 的运行时验证仍阻塞 |
...
> 不读 `RUST_LOG`）。**`daemon.rs` 的 macOS 接线已实现（2026-08-13，编译验证）**：
> engine 后端泛化（`Arc<dyn WgBackend>` 共享实例）+ `backend::platform_default()` 工厂
> + `http::router` 接收共享后端，打洞循环在 macOS 上走 userspace（boringtun）后端；
> `set_peer_endpoint` 缺口已于 2026-08-13 补上 remove+re-add 重建路径（ADR-0007
> 「代价与再评估」）。剩余阻塞仅为真实 utun + root 的运行时验证（须 macOS 真机/CI）——
> one-shot `hextet up` 又因 utun 随进程销毁无法常驻（ADR-0009 决策 5），须等真实 utun
> （root/CI）后本单元才真正可跑。plist 与指南里均已如实标注，不暗示可用。
