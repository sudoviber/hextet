...
- CI 新增 `check-windows` job（`windows-latest` 上 `cargo clippy --workspace --all-targets -- -D warnings`）：`cfg(not(linux|macos))` 的桩代码（daemon 占位、platform 的 `Unsupported` stub）此前无任何 CI 覆盖，本 job 用真实 MSVC 目标编译验证（M6 切片 D 的编译前置；只 check 不跑 test，Linux 独占运行时在 Windows 无法运行）。
