...
- Windows 平台能力第一片（ADR-0011 决策 1/4）：`crates/platform` 的 TUN 设备抽象扩展到 Windows（`tun` crate 的 wintun 分支，与 macOS/Linux 共用同一 `configure`/`create`/`AbstractDevice` 安全 API，运行时需 wintun.dll）；新增 `windows.rs` 实现 `list_global_ipv6`（`GetAdaptersAddresses` 枚举全局 IPv6、`is_usable_endpoint_addr` 过滤、按 FriendlyName/AdapterName 排除 hextet 接口，最小 unsafe 圈定 + `#![allow(unsafe_code)]`），其余 6 个平台函数仍为 `Unsupported` 桩（后续切片）。诚实边界：本机 `x86_64-pc-windows-gnu` check/clippy 编译通过，真实适配器枚举/路由/地址配装运行时待 Windows 真机或 CI 验证。
