...
### Added
- `crates/wg-userspace/src/raw_fd.rs`（M7 切片 C 的 Android 数据面核心）：`RawFdTun` 把裸 fd 上的 IP 包流适配成 gotatun 的 `IpSend`/`IpRecv`（非拥有式 fd 包装 + tokio AsyncFd + 非阻塞 fcntl，unsafe 圈在三处 fd 系统调用）。socketpair 单测验证 send/recv 双向往返（无需 root）。这是 Android `VpnService.Builder.establish()` fd 的前置；`apply` 走 `with_ip` 的接线待定「如何注进 DeviceSpec」。
