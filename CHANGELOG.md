...
### Fixed
- 单候选 peer 永久卡在 `Probing`（`crates/engine/src/fsm.rs`，netns-e2e-gossip 的 `r→a 转介超时` 根因）：`PeerFsm.last_transition` 被两处误推——①本机地址变化的 `kick`（不改对端 endpoint 却推了 `last_transition`）②单候选时每 2.5s 的「轮换」（回到同一个候选却推了 `last_transition`）。后果：内核 WG 会话建立后 `last_handshake` 冻结，而 `last_transition` 越推越后，`Connected` 判定要求的 `last_handshake >= last_transition` 永远不成立——A↔R/B↔R 这类单候选连接永远停在 Probing（数据面其实已通，gossip 转介照常走）。修复：拆出 `last_rotate`（只管 2.5s 轮换节流），`last_transition` 只在真正换 endpoint/候选时推进（`set_candidates`/`retry_from`/`Connected→Probing`/多候选轮换）；`kick` 不再推进 `last_transition`；单候选轮换只发 `Nudge` 不再重复 `SetEndpoint`。补 `kick_after_handshake_does_not_stale_it` 单测、更新 `single_candidate_keeps_re_nudging_same_endpoint`。
...
- DHT 会合五处（`crates/discovery/src/client.rs`、`crates/core/src/addr.rs`、`crates/engine/src/dht.rs`，子代理 DHT 审查发现）：①**seq 溢出**——`recent.seq() + 1` 直接加，攻击者成员可把 seq 推到 `i64::MAX`，受害者下次发布 debug panic / release 回绕成 `i64::MIN` 后记录永久卡死。改为 `checked_add(1)`、到顶报错。②**读取路径不过滤 endpoint**——AEAD 只保证「成员写的」，不保证地址合法，恶意成员可塞 loopback/ULA/链路本地。`lookup` 现在与 publish 侧同一套 `is_usable_endpoint_addr` 过滤。③**IPv4-mapped 泄漏**——`::ffff:a.b.c.d` 原来能混进 endpoint 候选，`is_usable_endpoint_addr` 现在一并排除（IPv6-only 项目）。④**串行 lookup 饿死服务循环**——每轮查询整轮限时 20s 预算，超时放弃剩余 peer（下轮补查），不再把 `publish_tick`/`save_tick`/`kick_rx` 饿死。⑤**发布侧不封顶 endpoint 数**——`publish_own` 现在 `.take(MAX_PUBLISHED_ENDPOINTS=8)`，与候选上限一致、且把记录值稳在 BEP44 1000 字节内。
...
- 中继逃生舱两处断线自愈缺口（`crates/engine/src/daemon.rs`，子代理 relay 路径审查发现）：①**握手失效退回 Probing 后永久卡死**——`drive_relay` 的 Probing 分支原来 `session.is_some()` 就直接 return，中继数据面一断、握手过期后会话还挂着，既不再注册也不再续期，服务端按 TTL 剪掉会话后 relay-only 的 peer 就永远连不回来。修复：Probing 分支对「会话还在」的场景按续期节奏重新注册（升级回直连期间除外，`upgrade_pending` 时仍专心试直连）。②**relay 重启后会话端点变了被误判成直连升级成功**——`on_relay_registered` 拿到新端点的会话后只 `set_candidates`（Connected 下是 no-op），FSM 还连着旧端点，下一 tick `drive_relay` 看到「Connected 端点 != 会话端点」就把刚拿到的会话注销掉（~3 分钟断线 + 误导日志）。修复：会话端点变且 FSM 还 Connected 在旧会话端点上时，立刻 `retry_from` 切到新端点。补 3 个 `drive_relay`/`on_relay_registered` 状态机级单测；relay E2E 无回归。
- DDNS mock（`crates/discovery/src/ddns/node.rs` 的 webhook HTTP 接收端）的偶发失败：原来只做一次 `read()` 就应答，TCP 把请求头/体拆成两段时（`cargo test --workspace` 并行高负载下会触发）会读丢 body（记录没写进表）+ 带着未读数据关连接触发 RST（reqwest 侧报 "connection reset by peer"）。改为按 `Content-Length` 读全请求再应答，并显式 `set_nonblocking(false)`（监听 socket 非阻塞、accept 出来的连接在部分平台沿用非阻塞标志，body 未到时 `read` 会立刻回 `WouldBlock`）；补 `find_header_end`/`content_length` 纯函数单测 + 分段请求的回归测试。
- 中继升级回直连的确定性修复（`crates/engine/src/fsm.rs` + `daemon.rs`，netns-e2e-relay.sh 升级直连 30s 稳定超时的根因）：升级是**不对称**的——B 侧留在 `Connected` 走内核 roaming 跟随（成功并注销中继），A 侧被 `on_discovered` 的 `retry_from` 踢进 `Probing`，而 `Probing`→`Connected` 只认「新鲜握手」。中继会话还新鲜（<120s）时 `SetEndpoint`+`Nudge` 只让内核沿旧会话发加密包 roaming、不产生新握手，A 永远卡在 Probing。修复：新增 `Action::Rehandshake(ep)`（`remove_peer`+`add_peer` flush 旧会话，下一个 nudge 触发真正的新握手），`retry_from_flush` 仅在**中继升级**两处（`on_discovered` 升级分支 + `drive_relay` 重试）用它；双侧换址恢复（DHT/LAN）仍走 `retry_from`（`SetEndpoint` 增量 roaming）——双侧同时换址时强制新握手会与对端自己的换址竞态，反而不稳。`add_peer` 重加用完整 `allowed_ips_for(site, routes)` + 原 keepalive。`PeerRuntime` 增 `keepalive` 字段。补 `retry_from_flush_emits_rehandshake` / `rehandshake_readds_with_full_allowed_ips_and_keepalive` 单测；relay E2E 升级直连 30s→2s 确定性通过，DHT 换址恢复无回归。
...
### Added
- FFI 首启引导（`join`/`init`，M7 切片 B 的配置引导缺口）：把 `hextet join`/`init` 的编排逻辑下沉到 `hextet_core::bootstrap`（`join_network`/`init_network`，纯编排：解码 invite → 验期 → 加载或生成身份 → 派生地址 + subnet 碰撞检查 → 渲染并原子写配置 0600），CLI 与 FFI 共用一份实现（不再「两个真相源」）。FFI 新增 `join(token, out_dir)`/`init(name, out_dir)`（UDL + `engine-ffi`，返回 `JoinOutcome`/`InitOutcome` JSON 或 `{"error":...}`），Android App 从此可全自助入网。CLI 行为不变（`--key-file`/`--out` 保留完整路径语义：跨目录密钥写绝对 `key_file`、同目录写相对名，`load_config_and_identity` 两路都正确解析；`init` 仍要求密钥先由 `keygen` 生成）。补 bootstrap 单测（join 复用密钥/拒覆盖/移除孤儿密钥/过期/坏 token，init 跨目录密钥解析 round-trip、同目录相对名、拒覆盖）。
- Android Kotlin `VpnService` 壳（M7 切片 B 的 Kotlin 侧，`apps/android/`）：完整 Gradle + Kotlin 项目——前台 `VpnService`（`Builder.establish()` → `detachFd()` → FFI `daemon_spawn_with_fd`，`onDestroy`/`onRevoke` 优雅 `daemon_shutdown`）、`MainActivity`（`POST_NOTIFICATIONS` + `VpnService.prepare` 授权 + 启停/状态）、`HextetClient`（UniFFI 生成的 `uniffi.hextet` 绑定 + `org.json` 解析）、`HextetApplication`（`System.loadLibrary("hextet_engine_ffi")`）。构建走 cargo-ndk 产出 `libhextet_engine_ffi.so` 到 `jniLibs/` + Gradle `Exec` 任务调 `uniffi-bindgen generate --library ...` 生成 Kotlin 绑定（官方推荐的 Exec-task 路线，无第三方 UniFFI 插件）。诚实边界：本机无 Android SDK，Kotlin 按已验证的 FFI 接口与 API 26–35 语义写成、**待有 SDK 的机器编译验证**；`hextet join`/`init` 走 FFI 是后续切片（本片只支持预置 `filesDir/hextet.toml` + `node.key`），VPN 单一槽位冲突（同 Tailscale）已写入 README。
- 按需连接的空闲零流量（M7 切片 D 第三片）：`keepalive = 0` 的 peer 现在**不主动发 nudge**（`PeerRuntime.on_demand`，`apply_actions` 里跳过 `Nudge`），endpoint 照常轮换/跟随、出站流量触发 WG 按需握手、FSM 观察到新鲜握手后照常回 `Connected`——把 on-demand 从「每 180s 一次活体探测」进一步推到「真正空闲零流量」省电。默认（`keepalive = 25`）行为不变。
- `[node] admin_keys` 白名单（gossip 准入/吊销授权，M3-D 计划项落地）：空白名单 = 任何成员都能签 `Member`/`Revocation`（默认，向后兼容）；设了之后只有列出的 admin 公钥签的条目才被采纳（`engine::gossip::handle_datagram` 在 merge 前拦一道）。堵住「任何成员都能吊销 admin / 准入任意节点」的缺口；**须全网一致**（同 network key）。补 config 解析 + 非 admin 条目被拒的单测。
