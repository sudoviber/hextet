...
- `daemon::spawn_with_fd(config_path, fd, mtu)`（M7 切片 B 的 Rust 侧接线）：daemon 主循环抽 `Transport` 枚举（`Platform` = 平台默认命名 TUN / `Fd` = VpnService fd），`run_async` 按 transport 分支——`Fd` 走 `apply_with_fd` 且跳过 `setup_interface`（VpnService 已配好地址/MTU）；engine 的 `hextet-wg-userspace` 依赖放宽到 `cfg(any(unix, windows))`（Linux/Android 也要用 `UserspaceBackend`）。FFI/Kotlin 侧把 VpnService fd 传进来即完成 Android 数据面接线。
