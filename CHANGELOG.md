...
### Changed
- Windows daemon 接线（ADR-0012 后数据面已跨平台）：`crates/engine` 的 `backend`/`daemon` 模块 `cfg` 扩到 Windows（`backend::platform_default()` 在 Windows 上返回 gotatun `UserspaceBackend`；`wg-userspace` 目标依赖扩到 macos+windows）；`daemon`/`dht` 的 SIGTERM 处理 `cfg(unix)` 门控 + 跨平台 terminate future（Windows 只有 Ctrl+C）。`check-windows` CI 移除 `--exclude hextet-wg-userspace`（gotatun 迁移后已跨平台）。诚实边界：`hextet up/status/down` 命令的 CLI `cfg` 仍 linux/macos（Windows 如实 bail），`hextet daemon` 已跨平台编译（走 `engine::daemon`）；本机 macOS/Linux/`x86_64-pc-windows-gnu`（platform）编译验证，engine/cli 的 Windows MSVC 编译交 `check-windows` CI。
