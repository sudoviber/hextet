...
### Changed
- `state.json` 版本升到 7：`PeerState` 新增 `last_handshake_secs`/`rx_bytes`/`tx_bytes`（WG 统计进 state.json，跨进程 CLI status / 跨线程 FFI status 不再需要访问进程内 gotatun 后端；daemon `tick_once` 把 `backend.status()` 合并进 `PeerState`）。
