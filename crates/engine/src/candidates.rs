...
///
/// 阶段 B 有 LAN 组播，阶段 D 有 gossip 转介，阶段 E 有 DHT，阶段 F 有自托管 DDNS；
...
pub enum Source {
    ...
    Gossip,
    /// DHT 会合（公共汇合点，控制面弱依赖 IPv4 出站）。
    Dht,
    /// 自托管 DDNS 会合（用户自己的域名 + 注册商 API，中国网络下可达性最好的兜底）。
    Ddns,
}
...
impl Source {
    ...
    /// 候选组装时的来源优先级（数字越小越靠前）。
    ///
    /// LAN（亲耳听到）先于 gossip（别人转告）先于 DHT（公共汇合点）先于 DDNS
    /// （用户自己的域名）。
